...
function renderAI() {
  ...
  /* API KEY SETUP — shows at top always */
  var hasKey = state.anthropicKey && state.anthropicKey.startsWith('sk-ant-');
  h += '<div style="background:' + (hasKey ? '#0a1a0a' : '#1a0a00') + ';border:1px solid ' + (hasKey ? '#14532d' : '#d97706') + ';border-radius:10px;padding:12px;margin-bottom:14px;display:flex;align-items:center;gap:10px;">';
  h += '<span style="font-size:18px;flex-shrink:0;">' + (hasKey ? '✅' : '🔑') + '</span>';
  h += '<div style="flex:1;min-width:0;">';
  if (hasKey) {
    h += '<div style="font-size:12px;font-weight:700;color:#10b981;">API Key connected — All AI features active!</div>';
    h += '<div style="font-size:10px;color:#556080;">sk-ant-....' + state.anthropicKey.slice(-8) + ' · <span onclick="clearApiKey()" style="cursor:pointer;color:#ef4444;">Remove key</span></div>';
  } else {
    h += '<div style="font-size:12px;font-weight:700;color:#f59e0b;">Add your Anthropic API key to activate AI</div>';
    h += '<div style="font-size:10px;color:#8899bb;margin-bottom:8px;">Get free key at console.anthropic.com → API Keys → Create key</div>';
    h += '<div style="display:flex;gap:6px;">';
    h += '<input id="api-key-input" type="password" placeholder="Paste your sk-ant-... key here" style="flex:1;font-size:11px;" />';
    h += '<button onclick="saveApiKeyFromInput()" class="btn-primary" style="font-size:11px;padding:6px 12px;flex-shrink:0;">Save Key</button>';
    h += '</div>';
  }
  h += '</div></div>';
...
function saveApiKeyFromInput() {
  ...
  if (!el || !el.value.trim()) { showToast('Please paste your API key', 'error'); return; }
  var key = el.value.trim();
  if (!key.startsWith('sk-ant-')) { showToast('Invalid key — must start with sk-ant-', 'error'); return; }
  window.AppState.anthropicKey = key;
  saveData();
  showToast('API key saved! All AI features now active ✅');
  renderPage();
}

function clearApiKey() {
  if (!confirm('Remove API key?')) return;
  window.AppState.anthropicKey = '';
  saveData();
  renderPage();
}
